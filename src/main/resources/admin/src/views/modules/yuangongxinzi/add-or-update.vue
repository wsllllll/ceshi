		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="绩效奖金" prop="jixiaojiangjin" >
					<el-input-number v-model="ruleForm.jixiaojiangjin" placeholder="绩效奖金" :disabled="ro.jixiaojiangjin"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="绩效奖金" prop="jixiaojiangjin" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="岗位补贴" prop="gangweibutie" >
					<el-input-number v-model="ruleForm.gangweibutie" placeholder="岗位补贴" :disabled="ro.gangweibutie"></el-input-number>
				</el-form-item>
				...
				<el-form-item class="input" v-if="type!='info'" label="假期扣薪" prop="jiaqikouxin" >
					<el-input v-model="jiaqikouxin" placeholder="假期扣薪" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.jiaqikouxin" label="假期扣薪" prop="jiaqikouxin" >
	...
	export default {
		...
		computed: {
			...
			jiaqikouxin : {
				get: function () {
					let c = this.ruleForm
					let a = c.qingjiatianshu*100
					this.ruleForm.jiaqikouxin = a?Number(Number(a).toFixed(2)):0
			...
			shifagongzi : {
				get: function () {
					let c = this.ruleForm
					let a = c.jibengongzi+c.jiabangongzi+c.jixiaojiangjin+c.gangweibutie-c.koukuanjine-c.jiaqikouxin
					this.ruleForm.shifagongzi = a?Number(Number(a).toFixed(2)):0
					return a?Number(Number(a).toFixed(2)):0
				}
			},
		...
		methods: {
			...
			xingmingChange () {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						if(data.data.qingjiatianshu){
							this.ruleForm.qingjiatianshu = data.data.qingjiatianshu
						}
					} else {
						this.$message.error(data.msg);
					}
			...
			async onSubmit() {
					...
					if(this.ruleForm.jiaqikouxin==0){
						this.$message.error('假期扣薪不能为空')
						return false
					}
