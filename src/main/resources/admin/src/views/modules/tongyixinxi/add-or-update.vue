	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="同意时间" prop="tongyishijian" >
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.tongyishijian" 
						type="datetime"
						:readonly="ro.tongyishijian"
						placeholder="同意时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.tongyishijian" label="同意时间" prop="tongyishijian" >
					<el-input v-model="ruleForm.tongyishijian" placeholder="同意时间" readonly></el-input>
				</el-form-item>
...
<script>
	export default {
		...
		created() {
			this.ruleForm.tongyishijian = this.getCurDateTime()
		},
		methods: {
			imgPreView(url){
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='tongyishijian'){
							this.ruleForm.tongyishijian = obj[o];
							this.ro.tongyishijian = true;
							continue;
						}
						if(o=='guanlizhanghao'){
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `tongyixinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.tongyixinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
